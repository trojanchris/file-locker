class FileLocker {
  ...
  constructor(persistent = false, bytes = 5 * 1024 * 1024) {
    if (persistent)
      ...
      navigator.webkitTemporaryStorage.requestQuota(bytes, grantedBytes =>
        window.webkitRequestFileSystem(
          TEMPORARY,
          grantedBytes,
          fs => (this.fs = fs)
  ...
  saveFile(path, data, callback) {
    ...
    this.fs.root.getFile(
      ...
      {},
      fileEntry =>
        this.removeFile(path, result => createFile(path, data, callback)),
      fileEntry => createFile(path, data, callback)
    );
  }
  ...
  saveImage(path, url, callback) {
    ...
    this.fs.root.getFile(
      ...
      {},
      fileEntry =>
        this.removeFile(path, result => createImage(path, url, callback)),
      fileEntry => createImage(path, url, callback)
    );
  }
